...
describe('DetectTokensController', function () {
  ...
  it('should check and add tokens while on main network', async function () {
    sandbox.useFakeTimers();
    network.setProviderType(MAINNET);
    while (network.getNetworkState() === 'loading') await Promise.resolve();
    const controller = new DetectTokensController({
      preferences,
      network,
      keyringMemStore,
    ...
    const erc20ContractAddresses = contractAddresses.filter(
      (contractAddress) => 
        contracts[contractAddress].erc20 === true &&
        !contracts[contractAddress].tnt20,
    );
    ...
    const got = preferences.store.getState().tokens;
    assert.equal(JSON.stringify(got), JSON.stringify(expect));
  });
