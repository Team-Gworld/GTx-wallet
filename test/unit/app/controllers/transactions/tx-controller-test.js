...
const txOptionsForEip155 = { common: new Common({ chain: Number(currentNetworkId), hardfork: 'berlin' }) };
...
describe('Transaction Controller', function () {
  ...
  beforeEach(function () {
    ...
    const getCurrentChainId = () => {
      const networkId = parseInt(networkStore.getState(), 10);
      return networkId ? ethUtil.addHexPrefix(networkId.toString(16)) : undefined;
    }
    txController = new TransactionController({
      ...
      getCurrentChainId,
      getSelectedNative: () => false,
      gasPricingTracker: {
        getCurrentNetworkEip1559Compatible: () => false,
        getBasicGasEstimates: () => { return { safeLow: 78, average: 80, fast: 82 } },
      },
    });
  ...
  describe('#sign replay-protected tx', function () {
    it('prepares a tx with the chainId set', async function () {
      ...
      const ethTx = new EthTx(ethUtil.toBuffer(rawTx), txOptionsForEip155);
  ...
  describe('#createSpeedUpTransaction', function () {
    ...
    it('should call this.addTx and this.approveTransaction with the expected args', async function () {
      ...
      const { lastGasPriceParams, type } = addTxArgs;
      assert.deepEqual(
        { lastGasPriceParams, type },
        {
          lastGasPriceParams: { gasPrice: '0xa' },
    ...
    it('should return the expected txMeta', async function () {
      ...
      const { lastGasPriceParams, type } = result;
      assert.deepEqual(
        { lastGasPriceParams, type },
        {
          lastGasPriceParams: { gasPrice: '0xa' },
  ...
  describe('# _determineTransactionType', function () {
    ...
    it('should return a token transfer type when data is for the respective method call', async function () {
      ...
      assert.deepEqual(result, {
        type: TRANSACTION_TYPE.TOKEN_METHOD_TRANSFER,
    ...
    it('should return a token approve type when data is for the respective method call', async function () {
      ...
      assert.deepEqual(result, {
        type: TRANSACTION_TYPE.TOKEN_METHOD_APPROVE,
        getCodeResponse: undefined,
      });
    });
    ...
    it('should return a contract deployment type when to is falsy and there is data', async function () {
      ...
      assert.deepEqual(result, {
        type: TRANSACTION_TYPE.DEPLOY_CONTRACT,
        getCodeResponse: undefined,
      });
    });
    ...
    it('should return a contract interaction type with the correct getCodeResponse when to is truthy and there is data and it is not a token transaction', async function () {
      ...
      assert.deepEqual(result, {
        type: TRANSACTION_TYPE.CONTRACT_INTERACTION,
