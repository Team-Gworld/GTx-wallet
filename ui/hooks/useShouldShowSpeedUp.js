...
export function useShouldShowSpeedUp(transactionGroup, isEarliestNonce) {
  ...
  const [speedUpEnabled, setSpeedUpEnabled] = useState(() => {
    return (
      ...
      isEarliestNonce &&
      !hasRetried &&
      initialTransaction?.metamaskNetworkId !== THETAMAINNET_NETWORK_ID
    ); // TODO: add fixedGasPrice to network properties and use that here instead (and in other places)
  });
  useEffect(() => {
