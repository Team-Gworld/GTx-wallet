...
export function isThetaNetwork(netIdOrChainId) {
  return [THETAMAINNET_NETWORK_ID, THETAMAINNET_CHAIN_ID].includes(
    netIdOrChainId,
  );
}
...
export function isTokenIrremovable(tokenAddress, isNativeAsset = false) {
  ...
  return (
    isNativeAsset ||
    Object.values(gtxTokens).find(
      (t) => t.address.toLowerCase() === lcTokenAddress,
    )?.autoAdd ||
    Object.values(thetaTokens).find(
      (t) => t.address.toLowerCase() === lcTokenAddress,
    )?.autoAdd
  );
}
