...
export function unlockHardwareWalletAccount(index, deviceName, hdPath, hdPathDescription) {
  log.debug(
...
export function signNftTx(
  ...
  return (dispatch) => {
    ...
    token.transferFrom(fromAddress, toAddress, tokenID, txData).catch((err) => {
      dispatch(hideLoadingIndication());
      dispatch(displayWarning(err.message));
    });
    dispatch(showConfTxPage());
...
export function updateGasData({
  ...
  return (dispatch) => {
    ...
    return estimateGas({
      ...
      value,
      estimateGasPrice:
        (gasPriceParams?.maxFeePerGas ?? gasPriceParams?.gasPrice) || '0x0',
      data,
      network,
      selectedNative,
    })
...
export function updateSendTokenBalance({ sendAsset, tokenContract, address }) {
  return (dispatch) => {
...
export function addTokens(tokens) {
  return (dispatch) => {
    if (Array.isArray(tokens)) {
      return Promise.all(
        tokens.map(({ address, symbol, decimals, image, logo }) =>
    ...
    return Promise.all(
      Object.entries(tokens).map(
        ([_, { address, symbol, decimals, image, logo }]) =>
          dispatch(addToken(address, symbol, decimals, image ?? logo)),
      ),
    );
  };
...
export function setPendingTokens(pendingTokens) {
  ...
  const tokens =
    address && symbol && decimals >= 0 && decimals <= 36
