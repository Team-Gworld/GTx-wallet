...
export default class TransactionStateManager extends EventEmitter {
  ...
  */
  ...
  getNetworkTxList(
    ...
    return this.getFullTxList().filter((transaction) => {
      const nativeMatch =
        Boolean(selectedNative) ===
        Boolean(transaction.txParams?.isThetaNative);
      ...
      switch (key) {
        case 'chainId':
          if (typeof value !== 'number' && typeof value !== 'string') {
            throw new Error(
              `chainId in txParams is not a Number or hex string. got: (${value})`,
            );
          }
          break;
        case 'isThetaNative':
          if (value !== true && value !== undefined) {
            throw new Error(
              `isThetaNative in txParams should be 'true' or 'undefined'. got: (${value})`,
            );
          }
          break;
        case 'thetaTxType':
          if (
            ...
            throw new Error(
              `thetaTxType must be a number between 0 and 10, or undefined. got: (${value})`,
            );
          }
          break;
        case 'additional':
          if (typeof value !== 'object' && value !== undefined) {
            throw new Error(
              `additional must be an object or undefined. got: (${value})`,
            );
          }
          break;
        default:
  ...
  */
  wipeTransactions(address) {
    const networkId = this.getNetwork();
    const selectedNative = this.getSelectedNative();
    const otherAccountOrNetTxs = this.getFullTxList().filter((transaction) => {
      const nativeMatch =
        Boolean(selectedNative) ===
        Boolean(transaction.txParams?.isThetaNative);
      let netMatch, isThetaTx;
      netMatch = transaction.metamaskNetworkId === networkId;
      isThetaTx = transaction.metamaskNetworkId === THETAMAINNET_NETWORK_ID;      
      return !(netMatch && (nativeMatch || !isThetaTx || !matchNative)) ||
        !(transaction.txParams.from === address);
    });

    // Update state
    this._saveTxList(otherAccountOrNetTxs);
  }
