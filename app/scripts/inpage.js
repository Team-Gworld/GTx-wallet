...
if (
  window.ethereum &&
  Date.now() - (window.gtx_lastConflictAlert || 0) > alertInterval
) {
  // TODO: put PopOver or other non-interruptive message
  window.gtx_lastConflictAlert = Date.now();
  throw new Error(
    'Conflicting wallets detected by GTx Wallet. Please remove one and try again.',
  );
}
...
initializeProvider({
  jsonRpcStreamName: 'gtx-provider',
