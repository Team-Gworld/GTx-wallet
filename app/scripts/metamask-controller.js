...
import { WALLET_PREFIX } from './controllers/permissions/enums';
...
export default class MetamaskController extends EventEmitter {
  ...
  constructor(opts) {
    ...
    this.txController = new TransactionController({
      ...
      getSelectedNative: this.networkController.getSelectedNative.bind(
        this.networkController,
      ),
      switchToScMode: this.networkController.switchToScMode.bind(
        this.networkController,
      ),
      preferencesStore: this.preferencesController.store,
      txHistoryLimit: 40,
    ...
    this.messageManager = new MessageManager(); // TODO: update deprecated
    this.personalMessageManager = new PersonalMessageManager(); // TODO: update deprecated
    ...
    if (!initState.NetworkController && defaultProviderConfig.type === 'rpc') {
      const { rpcTarget, chainId, ticker, nickname, rpcPrefs } =
  ...
  /**
   * RPC engine middleware for requesting provider state
   *
   * @param req
   * @param res
   * @param {Function} - next
   * @param {Function} - end
   */
  async requestProviderState(req, res, next, end) {
    if (
      req.method === 'metamask_getProviderState' ||
      req.method === `${WALLET_PREFIX}getProviderState`
    ) {
      const result = await this.getProviderState(req.origin);
  ...
  async getProviderState(origin) {
    return {
      ...
      networkVersion: this.networkController.getNetworkState(),
    };
  }
  ...
  getApi() {
    ...
    return {
      ...
      setSelectedNFT: nodeify(
        preferencesController.setSelectedNFT,
        preferencesController,
      ),
      setSelectedNative: nodeify(
        networkController.setSelectedNative,
        networkController,
      ),
   ...
   *
   * @param {Object} msgParams - The params of the message to sign & return to the Dapp. (address)
   * @param {Object} req - (optional) the original request, containing the origin
   * Passed back to the requesting Dapp.
   */
  async newRequestEncryptionPublicKey(msgParams, req) {
    if (
      (await this.keyringController.getKeyringForAccount(msgParams)).type ===
      'Ledger Hardware'
    ) {
      return Promise.reject(
        new Error('eth_getEncryptionPublicKey unsupported by Ledger'),
      );
    }
  ...
  setupUntrustedCommunication(connectionStream, sender) {
    ...
    this.setupProviderConnection(mux.createStream('gtx-provider'), sender);
    this.setupPublicConfig(mux.createStream('gtxPublicConfig'));
  }
  ...
  setupTrustedCommunication(connectionStream, sender) {
    ...
    this.setupProviderConnection(mux.createStream('gtx-provider'), sender, true);
  }
  ...
  setupProviderEngine({
    ...
    // methods
    engine.push(
      this.requestProviderState.bind(this)
    );
